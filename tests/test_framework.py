...
def test_registry_creation():
    ...
    assert "unet" in registries.models
    assert "resunet" in registries.models
    assert "dense_unet" in registries.models
    assert "attention_unet" in registries.models
    assert "transbts" in registries.models
    assert "nestedformer" in registries.models
    assert "slim_unetr" in registries.models
    assert "segformer3d" in registries.models
    assert "nnunet" in registries.models
    assert "vtunet" in registries.models
    assert "swinunetr" in registries.models
    assert "baseline_V2" in registries.models
    assert "dgcmfnet_V2" in registries.models
    assert "shared_encoder_concat_unet" in registries.models
...
def test_dgcmfnet_uses_six_unet_levels_and_fggim_on_every_encoder_stage():
    ...
    model = DGCMFNet(
        ...
        cg_gfm_num_heads=4,
        cg_gfm_num_layers=1,
        cg_gfm_patch_size=(1, 1, 1),
        cg_gfm_pos_grid_size=(1, 1, 1),
    )
    model.eval()
...
def test_attention_unet_returns_framework_logits_shape():
    """Attention U-Net should expose framework-native dense logits."""
    ...
    batch = Batch(
        signal=torch.randn(1, 2, 32, 32, 32),
        label=torch.zeros(1, 32, 32, 32).long(),
        id=["s1"],
        meta=[{}],
    )
    with torch.no_grad():
    ...
    assert outputs.shape == (1, 3, 32, 32, 32)
    assert len(model.attention_gates) == 4


def test_unet_family_models_return_framework_logits_shape():
    """Framework-native U-Net variants should expose dense logits."""
    from models.unet_variants import DenseUNet3D, ResUNet3D, UNet3D
    ...
    models = [
        UNet3D(num_classes=3, num_modalities=2, base_ch=2, channel_multipliers=[1, 2, 4]),
        ResUNet3D(num_classes=3, num_modalities=2, base_ch=2, channel_multipliers=[1, 2, 4]),
        DenseUNet3D(
            num_classes=3,
            num_modalities=2,
            base_ch=2,
    ...
    for model in models:
        ...
        with torch.no_grad():
            outputs = model(batch)
        assert outputs.shape == (1, 3, 16, 16, 16)


def test_paper_unet_3d_preserves_original_level_widths_and_same_padded_shape():
    """The registered U-Net is the paper architecture lifted to 3D."""
    from models.unet_variants import PaperUNet3D

    model = PaperUNet3D(num_classes=3, num_modalities=2, bottleneck_dropout=0.0).eval()
    assert model.encoder_channels == (64, 128, 256, 512, 1024)
    assert len(model.pools) == len(model.upconvs) == len(model.decoder_blocks) == 4
    assert all(
        isinstance(layer, nn.Conv3d) and layer.kernel_size == (3, 3, 3) and layer.padding == (1, 1, 1)
        for block in model.encoder_blocks
        for layer in (block.net[0], block.net[2])
    )
...
def test_brats_region_outputs_postprocess_to_framework_probabilities():
    """NestedFormer-style TC/WT/ET outputs should feed existing 4-class BraTS metrics."""
    from tasks.segmentation import SegmentationTask

    task = SegmentationTask(num_classes=4, loss="brats_sigmoid_region_dice")
    outputs = torch.tensor(
        [[[[[-4.0]]], [[[4.0]]], [[[-4.0]]]]],
        dtype=torch.float32,
    )

    probs = task.postprocess_outputs(outputs)

    assert probs.shape == (1, 4, 1, 1, 1)
    assert torch.allclose(probs.sum(dim=1), torch.ones(1, 1, 1, 1), atol=1e-6)
    assert int(torch.argmax(probs, dim=1).item()) == 2
...
def test_dgcmfnet_v2_returns_framework_logits_shape():
    ...
    batch = Batch(
        signal=torch.randn(1, 2, 33, 33, 33),
        label=torch.zeros(1, 33, 33, 33).long(),
        id=["s1"],
        meta=[{}],
    )
    with torch.no_grad():
...
def test_dgcmfnet_v2_calls_fggim_cggfm_and_frequency_guidance():
    ...
    assert [recorder.calls for recorder in frequency_recorders] == [
        [((1, 8, 9, 9, 9), (1, 64, 2, 2, 2))],
        [((1, 4, 17, 17, 17), (1, 32, 3, 3, 3))],
        [((1, 2, 33, 33, 33), (1, 16, 5, 5, 5))],
    ]
...
def test_fggim_can_use_modality_embeddings_relation_bias_and_cross_modal_edges():
    ...
    gim = FineGrainedGraphInteractionModule(
        in_channels=4,
        embed_dim=8,
        num_heads=2,
        pool_size=(2, 2, 1),
...
def test_dgcmfnet_v2_before_fusion_config_builds_registered_model():
    """The before-fusion variant should guide skip features before decoder concatenation."""
    model_config = load_toml("configs/model.dgcmfnet_V2.before_fusion.toml")["model"]
    model_name = model_config.pop("name")
    model = create_default_registries().models.create(model_name, **model_config)

    assert model.decoder_frequency_position == "before_fusion"
    assert model.decoder.frequency_position == "before_fusion"
    assert model.decoder.frequency_stage_mapping == ((0, 2), (1, 3), (2, 4))
    assert model.decoder.frequency_guidance["0"].semantic_gate.in_channels == 1024
    assert model.decoder.frequency_guidance["0"].semantic_gate.out_channels == 128
...
def test_shared_concat_decoder_can_apply_frequency_guidance_before_fusion():
    ...
    decoder = SharedConcatDecoder(
        num_classes=4,
        num_modalities=2,
        base_ch=1,
        decoder_type="frequency_guided",
        frequency_position="before_fusion",
        frequency_gate_mode="residual",
    )
    recorders = []
    for key in ("0", "1", "2"):
    ...
    decoder_input = [
        torch.randn(1, 64, 1, 1, 1),
        torch.randn(1, 32, 2, 2, 2),
    ...
    assert [recorder.calls for recorder in recorders] == [
        [((1, 8, 8, 8, 8), (1, 64, 1, 1, 1))],
        [((1, 4, 16, 16, 16), (1, 32, 2, 2, 2))],
        [((1, 2, 32, 32, 32), (1, 16, 4, 4, 4))],
    ]


def test_baseline_v2_config_builds_plain_backbone():
    """The baseline_V2 TOML config should keep all DG-CMFNet modules disabled."""
    from models.shared_encoder_concat_unet import BaselineV2

    default_model = BaselineV2(num_classes=3, num_modalities=2, base_ch=1)
    assert default_model.fg_gim_stages == []
    assert default_model.use_cg_gfm is False
    assert default_model.decoder.frequency_stage_mapping == ()

    model_config = load_toml("configs/model.baseline_V2.toml")["model"]
    model_name = model_config.pop("name")
    model = create_default_registries().models.create(model_name, **model_config)

    assert model_name == "baseline_V2"
    assert model.fg_gim_stages == []
    assert len(model.fg_gims) == 0
    assert model.use_cg_gfm is False
    assert model.skip_fusion == "concat"
    assert model.unet_bottleneck_dropout == 0.2
    assert model.decoder.frequency_stage_mapping == ()
    assert len(model.decoder.frequency_guidance) == 0
    assert model.decoder.decoder_feature_channels == [1024, 512, 256, 128, 64, 32]


def test_baseline_v2_optional_concat_1x1_skip_fusion_forward_shape():
    """baseline_V2 should still support concat_1x1 when an ablation needs that control."""
    from models.shared_encoder_concat_unet import BaselineV2
    ...
    model = BaselineV2(
        ...
        fg_gim_stages=[],
        use_cg_gfm=False,
        skip_fusion="concat_1x1",
        unet_bottleneck_dropout=0.2,
        cg_gfm_embed_dim=8,
        cg_gfm_num_heads=2,
        cg_gfm_num_layers=1,
        cg_gfm_patch_size=(1, 1, 1),
    ...
    assert [fusion.out_channels for fusion in model.level_fusions] == [1, 2, 4, 8, 16, 32]
    assert model.decoder.decoder_feature_channels == [32, 16, 8, 4, 2, 1]
    assert isinstance(model.bottleneck_dropout, torch.nn.Dropout3d)
    assert model.bottleneck_dropout.p == 0.2
...
def test_segformer3d_adapter_returns_framework_logits_shape():
    """SegFormer3D adapter should expose framework-native dense logits."""
    from models.segformer3d_adapter import SegFormer3DAdapter

    model = SegFormer3DAdapter(
        num_classes=3,
        num_modalities=2,
        sr_ratios=[4, 2, 1, 1],
...
def test_swinunetr_adapter_returns_framework_logits_shape():
    """Swin UNETR adapter should expose framework-native dense logits."""
    import importlib.util

    import pytest

    if importlib.util.find_spec("monai") is None:
        pytest.skip("MONAI is not installed in this environment")

    from models.swinunetr_adapter import SwinUNETRAdapter
...
def test_swinbts_returns_direct_tc_wt_et_region_logits():
    """SwinBTS must implement its paper-specific region head, not SwinUNETR."""
    from models.swinbts import SwinBTSAdapter

    model = SwinBTSAdapter(
        num_modalities=2,
        img_size=(32, 32, 32),
        embed_dim=8,
        depths=[1, 1, 1, 1],
    ...
    assert outputs.shape == (1, 3, 32, 32, 32)
    assert model.region_order == ("tc", "wt", "et")


def test_swinbts_dice_ce_loss_uses_nested_bra_ts_regions():
    """The SwinBTS loss should be near zero for exact TC/WT/ET predictions."""
    ...
    assert SwinBTSDiceCELoss()(logits, targets).item() < 1e-3


def test_vtunet_adapter_returns_framework_logits_shape():
    """VT-UNet adapter should expose framework-native dense logits."""
    import os
    from pathlib import Path

    import pytest

    from models.vtunet_adapter import VTUNetAdapter

    source_root = Path(os.environ.get("VTUNET_SOURCE_ROOT", "external/VT-UNet/VTUNet"))
    if not source_root.exists():
        pytest.skip("External VT-UNet source tree is not available in this environment")
    ...
    batch = Batch(
        signal=torch.randn(1, 4, 128, 128, 128),
        label=torch.zeros(1, 128, 128, 128).long(),
        id=["s1"],
        meta=[{}],
    )
    with torch.no_grad():
    ...
    assert outputs.shape == (1, 4, 128, 128, 128)
...
def test_dgcmfnet_accepts_recommended_stagewise_fggim_parameters():
    ...
    model = DGCMFNet(
        ...
        num_modalities=4,
        base_ch=8,
        fg_gim_stages=[0, 1, 2, 3, 4, 5],
        fg_gim_embed_dims=[32, 64, 128, 128, 256, 256],
        fg_gim_num_heads=4,
        fg_gim_pool_sizes=[
            (8, 8, 8),
            (8, 8, 8),
    ...
    assert [
        model.fg_gims[str(stage)].in_channels for stage in model.fg_gim_stages
    ] == expected_channels
    assert [
        model.fg_gims[str(stage)].embed_dim for stage in model.fg_gim_stages
    ] == expected_embed_dims
    assert [
        model.fg_gims[str(stage)].pool_size for stage in model.fg_gim_stages
    ] == expected_pool_sizes
    assert [
        model.fg_gims[str(stage)].num_nodes for stage in model.fg_gim_stages
    ] == [512, 512, 64, 64, 8, 8]
...
def test_uahl_focal_term_uses_target_class_probabilities():
    ...
    confident_probs = torch.tensor([[[[[0.90, 0.02]]], [[[0.04, 0.03]]], [[[0.03, 0.92]]], [[[0.03, 0.03]]]]])
...
def test_dc_and_ce_accepts_framework_segmentation_label_shape():
    """nnFormer loss should accept framework labels shaped [B, D, H, W]."""
    from framework.losses.dc_and_ce import DcAndCeLoss

    loss_fn = DcAndCeLoss()
    logits = torch.randn(1, 4, 4, 4, 4, requires_grad=True)
    targets = torch.randint(0, 4, (1, 4, 4, 4))

    loss = loss_fn(logits, targets)
    loss_with_channel = loss_fn(logits, targets.unsqueeze(1))

    assert torch.isfinite(loss)
    assert torch.isfinite(loss_with_channel)


def test_brats_region_dice_loss_accepts_framework_logits_and_labels():
    """SegFormer3D-style region Dice should work with 4-class framework logits."""
    from framework.losses.brats_original import BraTSRegionDiceLoss

    loss_fn = BraTSRegionDiceLoss()
    logits = torch.randn(1, 4, 3, 3, 3, requires_grad=True)
    targets = torch.randint(0, 4, (1, 3, 3, 3))

    loss = loss_fn(logits, targets)

    assert torch.isfinite(loss)
    assert set(loss_fn.get_last_components()) == {"loss_dice"}


def test_brats_edice_loss_records_dice_and_bce_components():
    """VT-UNet EDice adaptation should expose Dice and BCE components."""
    from framework.losses.brats_original import BraTSEDiceLoss

    loss_fn = BraTSEDiceLoss()
    logits = torch.randn(1, 4, 3, 3, 3, requires_grad=True)
    targets = torch.randint(0, 4, (1, 3, 3, 3))
    ...
    assert torch.isfinite(loss)
    assert set(components) == {"loss_dice", "loss_bce"}


def test_transbts_softmax_dice_supervises_background():
    """TransBTS Dice should penalize predictions that collapse background into class 1."""
    from framework.losses.brats_original import TransBTSSoftmaxDiceLoss

    loss_fn = TransBTSSoftmaxDiceLoss()
    targets = torch.zeros(1, 2, 2, 2, dtype=torch.long)
    correct_logits = torch.full((1, 4, 2, 2, 2), -10.0)
    correct_logits[:, 0] = 10.0
    collapsed_logits = torch.full((1, 4, 2, 2, 2), -10.0)
    collapsed_logits[:, 1] = 10.0

    correct_loss = loss_fn(correct_logits, targets)
    collapsed_loss = loss_fn(collapsed_logits, targets)
    components = loss_fn.get_last_components()

    assert loss_fn.classes == (0, 1, 2, 3)
    ...
    assert components["loss_dice_class_0"] > 0.99
    assert collapsed_loss > correct_loss


def test_multilabel_bce_focal_gamma_zero_matches_bce_with_logits():
    """Attention U-Net default focal gamma=0 should reduce to BCEWithLogits."""
    from framework.losses.brats_original import MultiLabelBCEFocalLoss

    loss_fn = MultiLabelBCEFocalLoss(num_classes=4, gamma=0.0)
    logits = torch.randn(1, 4, 2, 2, 2, requires_grad=True)
    targets = torch.randint(0, 4, (1, 2, 2, 2))

    loss = loss_fn(logits, targets)
    one_hot = torch.nn.functional.one_hot(targets, num_classes=4).movedim(-1, 1).to(logits.dtype)
...
def test_frequency_guided_decoder_can_run_after_concat_before_conv():
    ...
    assert logits.shape == (1, 4, 32, 32, 32)
    assert calls == [
        [((1, 16, 8, 8, 8), (1, 64, 1, 1, 1))],
        [((1, 8, 16, 16, 16), (1, 32, 2, 2, 2))],
        [((1, 4, 32, 32, 32), (1, 16, 4, 4, 4))],
    ]
...
def test_shared_concat_decoder_can_guide_skips_before_fusion():
    ...
    decoder = SharedConcatDecoder(
        num_classes=4,
        base_ch=1,
        level_channels=[1, 2, 4, 8, 16, 32],
        decoder_type="frequency_guided",
        frequency_position="before_fusion",
        frequency_stage_mapping=[[0, 2], [1, 3], [2, 4]],
        frequency_gate_mode="residual",
    )
    recorders = []
    for key in ("0", "1", "2"):
    ...
    assert logits.shape == (1, 4, 32, 32, 32)
    assert [recorder.calls for recorder in recorders] == [
        [((1, 4, 8, 8, 8), (1, 32, 1, 1, 1))],
        [((1, 2, 16, 16, 16), (1, 16, 2, 2, 2))],
        [((1, 1, 32, 32, 32), (1, 8, 4, 4, 4))],
    ]


def test_configurable_unet3d_is_registered_for_legacy_snapshots():
    """Older experiment snapshots should resolve the configurable UNet3D explicitly."""
    assert "unet3d" in create_default_registries().models


def _run_recording_decoder(
    frequency_position=None,
) -> tuple[
    torch.nn.Module,
...
def test_trainer_prints_and_logs_uahl_loss_components(tmp_path, capsys):
    ...
    trainer = Trainer(
        ...
        task=task,
        optimizer=optimizer,
        train_loader=[batch([0, 1]), batch([2, 3])],
        val_loader=[batch([1, 3])],
        test_loader=[batch([0, 3])],
        train_metrics=MetricCollection([]),
        val_metrics=MetricCollection([]),
        test_metrics=MetricCollection([]),
        settings=TrainerSettings(epochs=1, patience=5, device="cpu"),
        checkpoint_manager=CheckpointManager(tmp_path / "ckpt", monitor="loss", mode="min"),
        logger=logger,
        run_dir=tmp_path,
    )
...
def test_trainer_runs_one_epoch(tmp_path):
    ...
    assert "accuracy_accuracy" in metrics  # prefixed by collection
...
def test_full_volume_metrics_penalize_large_empty_et_prediction_with_brats_diagonal():
    ...
    assert abs(metrics["et_hd95"] - BRATS_EMPTY_HD95_PENALTY) < 1e-6
...
def test_sliding_window_predict_proba_uses_task_postprocess():
    ...
    probs = sliding_window_predict_proba(
        ...
        num_classes=4,
        window_size=(4, 4, 4),
        postprocess_fn=four_class_postprocess,
    )
...
def test_mc_dropout_predict_returns_mean_probs_and_uncertainty():
    ...
    assert float(result.epistemic_uncertainty.mean()) > 0.20
...
def test_summarize_mc_sample_metrics_reports_mean_and_std():
    ...
    summary = summarize_mc_sample_metrics(
        [
            {"mean_dice": 0.5, "wt_dice": 1.0, "tc_dice": 0.25},
            {"mean_dice": 1.0, "wt_dice": 0.0, "tc_dice": 0.75},
        ]
    )
...
def test_summarize_uncertainty_reports_paper_aligned_names():
    ...
    result = MCDropoutPrediction(
        pred=torch.zeros(1, 1, 1, dtype=torch.long),
        mean_probs=torch.full((2, 1, 1, 1), 0.5),
        epistemic_uncertainty=torch.full((1, 1, 1), 0.25),
        aleatoric_uncertainty=torch.full((1, 1, 1), 0.01),
        sample_count=2,
    )

    summary = summarize_uncertainty(result)

    assert abs(summary["aleatoric_uncertainty_mean"] - 0.01) < 1e-8
    assert abs(summary["epistemic_uncertainty_mean"] - 0.25) < 1e-8
